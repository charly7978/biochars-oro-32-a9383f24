...
export class ModularVitalSignsProcessor {
  ...
  processSignal(value: number): VitalSignsResult {
    ...
    const bp = this.bpProcessor ? this.bpProcessor.processValue(distributedSignals[VitalSignType.BLOOD_PRESSURE] || 0) : {
      systolic: 0,
      diastolic: 0
    };
    const glucose = this.glucoseProcessor ? this.glucoseProcessor.processValue(distributedSignals[VitalSignType.GLUCOSE] || 0) : 0;
    const hydration = this.hydrationProcessor ? this.hydrationProcessor.processValue(distributedSignals[VitalSignType.HYDRATION] || 0) : {
      totalCholesterol: 0,
      hydrationPercentage: 0
    };
    const cardiac = this.cardiacProcessor ? this.cardiacProcessor.processValue(distributedSignals[VitalSignType.HEARTBEAT] || 0) : {
      heartRate: 0,
      hrv: 0
    };
  ...
  reset(): void {
    ...
    if (this.signalDistributor) this.signalDistributor.reset();
   ...
   * Get an empty result
   */
  getEmptyResult(): VitalSignsResult {
    return {
      spo2: 0,
      pressure: '--/--',
      arrhythmiaStatus: '--',
